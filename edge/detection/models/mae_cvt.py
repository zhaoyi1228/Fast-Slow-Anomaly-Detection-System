"""
Masked Autoencoder with Convolutional Vision Transformer (MAE-CvT)
From aed-mae project - Self-Distilled Masked Auto-Encoders for Video Anomaly Detection
"""
...
from .cvt import ConvEmbed, Block
from .morphology import Erosion2d, Dilation2d
...
class MaskedAutoencoderCvT(nn.Module):
    def __init__(self, img_size=(512, 512), patch_size=16, in_chans=9, out_chans=4,
        ...
        # Ensure abnormal_score_func is a list or tuple
        if isinstance(abnormal_score_func, str):
            abnormal_score_func = [abnormal_score_func, abnormal_score_func]
        self.abnormal_score_func = abnormal_score_func[0]
        # Ensure abnormal_score_func_TS has default value
        self.abnormal_score_func_TS = abnormal_score_func[1] if len(abnormal_score_func) > 1 else abnormal_score_func[0]
        # --------------------------------------------------------------------------
        ...
        self.decoder_student_pred = nn.Linear(decoder_embed_dim, patch_size ** 2 * out_chans, bias=True)
    ...
    def abnormal_score_TS(self, imgs, pred_stud, pred_teacher, mask, gradients):
        ...
        if self.abnormal_score_func_TS == "L1":
            output.append(torch.abs(pred_teacher - pred_stud).mean((2)))
