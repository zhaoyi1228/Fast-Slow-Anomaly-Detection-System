...
from config import RELAY_SERVER, JIGSAW_SERVER, DETECTION_CONFIG, LOG_CONFIG
...
class EdgeDetector:
    ...
    def __init__(self, relay_host: str = None, relay_port: int = None,
                 jigsaw_host: str = None, jigsaw_port: int = None):
        """
        初始化端侧检测器
        ...
        Args:
            relay_host: 中间机器IP
            relay_port: 中间机器端口
            jigsaw_host: Jigsaw服务主机（默认本地）
            jigsaw_port: Jigsaw服务端口
        """
        # 通信客户端
        self.relay_client = RelayClient(relay_host, relay_port)
        self.jigsaw_client = create_jigsaw_client(
            jigsaw_host or JIGSAW_SERVER["host"],
            jigsaw_port or JIGSAW_SERVER["port"]
        )
    ...
    def start(self):
        ...
        # 检查连接
        print(f"\n检查Jigsaw服务 ({JIGSAW_SERVER['host']}:{JIGSAW_SERVER['port']})...")
        jigsaw_ok = self.jigsaw_client.health_check()
        print(f"Jigsaw服务状态: {'正常' if jigsaw_ok else '异常'}")
        ...
        if not jigsaw_ok:
            print("\n警告: Jigsaw服务未就绪，请先启动 Jigsaw 服务")
            print("启动命令: python start_edge.py --start-jigsaw")
    ...
    def _process_frame(self, frame_data: FrameData):
        ...
        # 步骤1: Jigsaw本地检测
        jigsaw_result = self.jigsaw_client.detect_single(frame_data.color_base64)

        if jigsaw_result.get("error"):
            self.stats["jigsaw_failed"] += 1
            print(f"[{self.frame_counter}] Jigsaw检测失败: {jigsaw_result['error']}")
            return

        self.stats["jigsaw_success"] += 1
        anomaly_score = jigsaw_result.get("anomaly_score", 1.0)
        spatial_score = jigsaw_result.get("spatial_score", 1.0)
        temporal_score = jigsaw_result.get("temporal_score", 1.0)
        ...
        print(f"[{self.frame_counter}] Jigsaw={anomaly_score:.3f} ({status}) "
    ...
    def stop(self):
        ...
        print(f"  Jigsaw成功: {self.stats['jigsaw_success']}")
        print(f"  Jigsaw失败: {self.stats['jigsaw_failed']}")
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "jigsaw_connected": self.jigsaw_client.health_check(),
            "stats": self.stats,
        }
...
def main():
    ...
    parser.add_argument('--relay-port', type=int, default=RELAY_SERVER["port"],
                        help='中间机器端口')
    parser.add_argument('--jigsaw-host', type=str, default=JIGSAW_SERVER["host"],
                        help='Jigsaw服务主机')
    ...
    detector = EdgeDetector(
        ...
        jigsaw_host=args.jigsaw_host,
        jigsaw_port=args.jigsaw_port
    )
