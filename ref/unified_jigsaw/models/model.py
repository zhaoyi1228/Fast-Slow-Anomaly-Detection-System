...
class WideBranchFeatureNet(nn.Module):

    def __init__(self, time_length=7, num_classes=[127, 8]):
        super(WideBranchFeatureNet, self).__init__()
...
if __name__ == '__main__':
    net = WideBranchNet(time_length=9, num_classes=[81, 81]) # 这里的9会报错，因为最后的MaxPool的time_length需<=输入中的T维度
    x = torch.rand(32, 3, 9, 64, 64)
    out = net(x)
    print(out[0].shape)
